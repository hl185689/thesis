...
final.md <- final.md %>% 
  mutate(hand_tuned_na = predict(m.final,newdata=final.md,type="response")) %>% 
  rename(fid=FID,
         ce_yn=CE_yn)
...
for.output <- for.output %>%
  mutate(hand_tuned = if_else(is.na(hand_tuned_na),
...
write_csv(for.output,
          paste0(here(),"/data/probability_comparison_all_nanona_hl.csv"))
