...
d <- read_csv(paste0(here(),"/data/probability_comparison_all_nanona_hl.csv")) 

d <- d %>% 
  mutate(combined = (step_wise_estimate + full_xgb_pred + hand_tuned + hand_tuned_na)/4)
...
plot(hand.roc.na,main="Hand-Tuned w NA",print.auc=T)
