...
df2 <- read.csv("logit variables.csv")

df <- df[,c(2,29,df2$varnum)] # adding in FID and CE_yn
head(df)

#need to plot histograms for all predictor variables to check for normality

par(mfrow=c(2,2))

for(i in 3:4){
  hist(df[,i])
} # columns 3 to 4 - show need for normalizing

#table(df[,15]) #looked at other variables and looked fine
...
sam.train <- sample(nrow(sec.sam.cen), 4030) # 4030 is 10x total number of CEs remaining
...
names(train.set) # going to be running models on columns 5:end for new log10 variables 


#test run 1 variable
m1 <- glm(CE_yn ~ train.set[,5], data=train.set, family = "binomial")
summary(m1)
